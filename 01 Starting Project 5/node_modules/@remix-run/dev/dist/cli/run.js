...
async function run(argv = process.argv.slice(2)) {
  ...
  // TODO: remove in v2
  if (flags["scheme"]) {
    logger.logger.warn("`--scheme` flag is deprecated", {
      details: ["Use `REMIX_DEV_ORIGIN` instead", "-> https://remix.run/docs/en/main/other-api/dev-v2#how-to-integrate-with-a-reverse-proxy"]
    });
  }
  // TODO: remove in v2
  if (flags["host"]) {
    logger.logger.warn("`--host` flag is deprecated", {
      details: ["Use `REMIX_DEV_ORIGIN` instead", "-> https://remix.run/docs/en/main/other-api/dev-v2#how-to-integrate-with-a-reverse-proxy"]
    });
  }
  if (flags["tls-key"]) {
  ...
  if (args["--no-restart"]) {
    logger.logger.warn("`--no-restart` flag is deprecated", {
      details: ["Use `--manual` instead.", "-> https://remix.run/docs/en/main/guides/development-performance#manual-mode"]
    });
    flags.manual = true;
